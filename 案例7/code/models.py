...
class ConvolutionalBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 batch_norm=False, activation=None,spectral_norm=True):
        """
...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, kernel_size=3, n_channels=64,spectral_norm=True):
        """
        :param kernel_size: kernel size
    ...
    # The first convolutional block
        self.conv_block1 = ConvolutionalBlock(in_channels=n_channels, out_channels=n_channels, kernel_size=kernel_size,
                                              batch_norm=True, activation='PReLu',spectral_norm=spectral_norm)
        # The second convolutional block
        self.conv_block2 = ConvolutionalBlock(in_channels=n_channels, out_channels=n_channels, kernel_size=kernel_size,
                                              batch_norm=True, activation=None,spectral_norm=spectral_norm)
...
class SRResNet(nn.Module):
    ...
    def __init__(self, large_kernel_size=9, small_kernel_size=3,
                 n_channels=64, n_blocks=16, scaling_factor=4,self_attention=False,spectral_norm=True,channel_attention=True):
        """
        ...
        self.conv_block1 = ConvolutionalBlock(in_channels=3, out_channels=n_channels, kernel_size=large_kernel_size,
                                          batch_norm=False, activation='PReLu',spectral_norm=spectral_norm)

        # A sequence of n_blocks residual blocks, each containing a skip-connection across the block
        self.res1=ResidualBlock(kernel_size=small_kernel_size,n_channels=n_channels,spectral_norm=spectral_norm)

        self.residual_blocks = nn.Sequential(
            *[ResidualBlock(kernel_size=small_kernel_size, n_channels=n_channels,spectral_norm=spectral_norm) for i in range(1,n_blocks)])
        ...
        if self.channel_attention:
            self.CA1=ChannelAttention(num_features=64, reduction=16)
            self.CA2 = ChannelAttention(num_features=64, reduction=16)
        ...
        self.conv_block3 = ConvolutionalBlock(in_channels=n_channels, out_channels=3, kernel_size=large_kernel_size,
                                              batch_norm=False, activation='Tanh',spectral_norm=spectral_norm)
    ...
    def forward(self, lr_imgs):
        ...
        if self.channel_attention:
            output = self.CA2(output)
        output = self.residual_blocks(output)  # (N, n_channels, w, h)
        if self.self_attention:
            output=self.self_attention_block_64_1(output)
        if self.channel_attention:
            output=self.CA1(output)
        output = self.conv_block2(output)  # (N, n_channels, w, h)
        output = output + residual  # (N, n_channels, w, h)
        if self.self_attention:
            output = self.self_attention_block_64_2(output)
        if self.channel_attention:
            output=self.CA2(output)
...
class Discriminator(nn.Module):
    ...
    def __init__(self, kernel_size=3, n_channels=64, n_blocks=8, fc_size=1024,
                 self_attention=False,spectral_norm=True,channel_attention=True):
        """
        ...
        self.conv1=ConvolutionalBlock(in_channels=3,out_channels=64,kernel_size=kernel_size,stride=1,batch_norm=False,
                                 activation='LeakyRelu',spectral_norm=spectral_norm)
        self.conv2=ConvolutionalBlock(in_channels=64,out_channels=64,kernel_size=kernel_size,stride=2,batch_norm=True,
                                 activation='LeakyRelu',spectral_norm=spectral_norm)
        in_channels=64
        ...
        if self.self_attention:
            self.self_attention_block_1 = SelfAttentionBlock(in_feature_maps=128)
            self.self_attention_block_2 = SelfAttentionBlock(in_feature_maps=32)
        if self.channel_attention:
            self.CA1 = ChannelAttention(num_features=64, reduction=16)
            self.CA2 = ChannelAttention(num_features=64, reduction=16)
            self.CA3 = ChannelAttention(num_features=128, reduction=32)
            self.CA4 = ChannelAttention(num_features=32, reduction=8)


        self.conv9=ConvolutionalBlock(in_channels=512, out_channels=128, kernel_size=3,
                           stride=2, batch_norm=True,activation='LeakyReLu',
                                      spectral_norm=spectral_norm)
        self.conv10=ConvolutionalBlock(in_channels=128, out_channels=32, kernel_size=3,
                           stride=2, batch_norm=True,activation='LeakyReLu',
                                      spectral_norm=spectral_norm)
        self.conv11=ConvolutionalBlock(in_channels=32, out_channels=1, kernel_size=3,
                           stride=2, batch_norm=False,activation=None,
                                      spectral_norm=False)
    ...
    def forward(self, imgs):
        ...
        output=self.conv9(output) #512->128
        # if self.self_attention:
        #     output=self.self_attention_block_1(output)
        # if self.channel_attention:
        #     output=self.CA3(output)
        output=self.conv10(output)

        # if self.self_attention:
        #     output=self.self_attention_block_2(output)
        # if self.channel_attention:
        #     output = self.CA4(output)
